...
def quantify_masks(intensity_image, mask_images, mask_names, channel_name, background_radius = None, quantify_foci = False):
    if not (background_radius is None):
        intensity_image = rolling_ball_filter(intensity_image.astype(np.float32), background_radius)
...
def process_patch(
    ...
    valid_labels = cells_in_box # Filter objects in non-offset box to avoid redundant quantification
    ...
    if(np.sum(np.ravel(mask_nucleus_original_filtered) > 0) == 0):
        return(None)

    selected_channels = ['DAPI', 'HMGA2', 'PanCK', 'uPAR', 'TNC', 'Arg1', 'GFP']
    selected_masks = [mask_cyto_filtered, mask_nucleus_original_filtered]
    mask_names = ["cyto", "nucleus"]
    background_radius = [10,10,10,5,5,10,None]
    
    result = []
    # Quantify all channels
    for current_channel, current_radius in zip(selected_channels, background_radius):
        current_intensity_image = get_image_slice(sdata, "celldive", "scale1", current_channel, row_slice, col_slice)
        df_intensity, df_objects = quantify_masks(current_intensity_image, selected_masks, mask_names, current_channel, current_radius)
...
if __name__ == '__main__':
    ...
    result_final.to_csv(os.path.join(output_dir, SLIDE_ID + "__measurements_background.csv.gz"))
